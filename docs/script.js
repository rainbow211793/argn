...
function getMediaPreview(item, isDetail = false) {
  ...
  if (item.type === 'image') {
    ...
    if (isDetail) {
      // Show embedded preview and open in new tab option
      return `
        <div style="display: flex; flex-direction: column; gap: 12px; align-items: center;">
          <iframe class="${baseClass}" src="${url}" sandbox="allow-same-origin allow-scripts" style="border: 1px solid #3a5a7a; border-radius: 8px; width: 100%; height: 400px;"></iframe>
          <a href="${url}" target="_blank" class="html-link" style="display: inline-block; padding: 12px 24px; background-color: #3a5a7a; color: white; text-decoration: none; border-radius: 8px; font-size: 16px;">Open HTML in New Tab</a>
        </div>
      `;
    }
